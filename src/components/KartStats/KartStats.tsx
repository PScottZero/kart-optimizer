...
export const KartStats: React.FC = () => {
  const speed = (data: PartData): number => {
    return data.selectedDriver.stats.groundSpeed +
  ...
  const acceleration = (data: PartData): number => {
    return data.selectedDriver.stats.acceleration +
  ...
  const weight = (data: PartData): number => {
    return data.selectedDriver.stats.weight +
  ...
  const handling = (data: PartData): number => {
    return data.selectedDriver.stats.groundHandling +
  ...
  const traction = (data: PartData): number => {
    return data.selectedDriver.stats.offRoadTraction +
      ...
      {context => (
        <div className="PartStats">
          <div className="StatsContainer">
            ...
            <ProgressBar label="Acceleration" value={acceleration(context)}></ProgressBar>
            <ProgressBar label="Weight" value={weight(context)}></ProgressBar>
            <ProgressBar label="Handling" value={handling(context)}></ProgressBar>
            <ProgressBar label="Traction" value={traction(context)}></ProgressBar>
          </div>
        </div>
      )}
    </PartContext.Consumer>
