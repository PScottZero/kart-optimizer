...
export default class KartCombo extends Component<KartComboProps> {
  ...
  partsForKart(): Part[] {
    return [
      this.context.drivers.find((driver: Part) => driver.name === this.props.kart.driver),
      this.context.bodies.find((body: Part) => body.name === this.props.kart.body),
      this.context.tires.find((tire: Part) => tire.name === this.props.kart.tire),
      this.context.gliders.find((glider: Part) => glider.name === this.props.kart.glider)
    ]
  }
}
